    ...
    *,
    ...
    for model_id, usage in models_usage.items():
        ...
        if pricing_row is None:
            ...
            continue
        prompt_rate = pricing_row.get("prompt")
        completion_rate = pricing_row.get("completion")
        if prompt_rate is None or completion_rate is None:
            missing.append(model_id)
            continue

        # OpenRouter pricing is expressed per 1M tokens.
        prompt_cost = (Decimal(prompt_tokens) / Decimal("1000000")) * prompt_rate
        completion_cost = (Decimal(completion_tokens) / Decimal("1000000")) * completion_rate
