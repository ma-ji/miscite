...
def _extract_text_direct(path: Path, *, backend: str) -> str:
    ...
    if backend == "docling":
        from server.miscite.analysis.extract.docling_extract import extract_docling_text

        return extract_docling_text(path)
    if backend == "markitdown":
        from server.miscite.analysis.extract.markitdown_extract import extract_markitdown_text
