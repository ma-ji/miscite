...
def parse_references_with_llm(
    ...
    *,
    ...
    if not text:
        notes.append("No references text provided for bibliography parsing.")
        return [], {}, notes
    ...
    try:
        ...
        logger.exception("LLM bibliography output invalid; proceeding without parsed references.")
        notes.append("LLM bibliography output invalid; proceeding without parsed references.")
        return [], {}, notes

    refs_raw = payload.get("references")
    if not isinstance(refs_raw, list):
        logger.error("LLM bibliography output missing 'references' list; proceeding without parsed references.")
        notes.append("LLM bibliography output missing references list; proceeding without parsed references.")
        return [], {}, notes
...
def extract_references_section_with_llm(
    ...
    *,
    ...
    try:
        ...
        logger.exception("LLM references-section output invalid; proceeding without references section.")
        notes.append("LLM references-section output invalid; proceeding without references section.")
        return None, notes
    refs_text = payload.get("references_text")
    if refs_text is not None and not isinstance(refs_text, str):
        logger.error("LLM references_text had invalid type; proceeding without references section.")
        notes.append("LLM references_text had invalid type; proceeding without references section.")
        return None, notes
    ...
    if conf is not None:
        try:
            ...
            logger.error("LLM references-section confidence invalid; ignoring.")
            conf_f = None
        if conf_f is not None and (conf_f < 0.0 or conf_f > 1.0):
            logger.error("LLM references-section confidence out of range; ignoring.")
...
def parse_citations_with_llm(
    ...
    *,
    ...
    if not text:
        notes.append("No main text provided for citation parsing.")
        return [], notes
    ...
    try:
        ...
        logger.exception("LLM citation output invalid; proceeding without parsed citations.")
        notes.append("LLM citation output invalid; proceeding without parsed citations.")
        return [], notes

    cits_raw = payload.get("citations")
    if not isinstance(cits_raw, list):
        logger.error("LLM citation output missing 'citations' list; proceeding without parsed citations.")
        notes.append("LLM citation output missing citations list; proceeding without parsed citations.")
        return [], notes
